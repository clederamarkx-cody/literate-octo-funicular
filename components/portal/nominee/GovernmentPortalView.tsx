    ...
    return (
        <div className="animate-in fade-in duration-500 space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden relative group">
                <div className="absolute top-0 left-0 w-2 bg-blue-600 h-full group-hover:w-3 transition-all"></div>
                <div className="p-8">
                    <div className="flex flex-col md:flex-row justify-between gap-10">
                        <div className="flex-1 space-y-8">
                            <div className="flex items-center space-x-5">
                                <div className="p-4 bg-blue-50 rounded-3xl text-blue-600 ring-1 ring-blue-100 transition-colors">
                                    <Building2 size={36} />
                                </div>
                                <div>
                                    <h2 className="text-3xl font-serif font-bold text-gkk-navy leading-tight">{nomineeData?.name || nomineeData?.organizationName}</h2>
                                    <p className="text-gray-500 flex items-center gap-2 mt-2 font-medium"><MapPin size={14} className="text-blue-500" /> {nomineeData?.details?.address}, {nomineeData?.region}</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                                <div className="space-y-2"><span className="text-[10px] font-bold text-gray-400 uppercase block">Agency Sector</span><div className="flex items-center gap-2 text-sm font-bold text-gkk-navy"><Briefcase size={16} className="text-blue-500" /> {nomineeData?.details?.industry || nomineeData?.industrySector}</div></div>
                                <div className="space-y-2"><span className="text-[10px] font-bold text-gray-400 uppercase block">Personnel Count</span><div className="flex items-center gap-2 text-sm font-bold text-gkk-navy"><Users size={16} className="text-blue-500" /> {nomineeData?.details?.employees || nomineeData?.workforceSize} Pax</div></div>
                                <div className="space-y-2"><span className="text-[10px] font-bold text-gray-400 uppercase block">Agency ID</span><div className="flex items-center gap-2 text-sm font-bold text-gkk-navy font-mono"><Hash size={16} className="text-blue-500" /> {nomineeData?.regId}</div></div>
                                <div className="space-y-2"><span className="text-[10px] font-bold text-gray-400 uppercase block">OSH Lead</span><div className="flex items-center gap-2 text-sm font-bold text-gkk-navy"><HardHat size={16} className="text-blue-500" /> {nomineeData?.details?.safetyOfficer}</div></div>
                            </div>
                        </div>
                        <StageProgress
            ...
            <div id="documents-section" className="space-y-8 pb-20">
                ...
                <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-sm">
                    <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                        <div>
                            ...
                            <p className="text-sm border-l-4 border-blue-600 pl-3 py-1 font-bold italic text-gkk-navy/80 bg-blue-50/50 mt-4">Public sector safety compliance and inter-agency OSH standard documentation.</p>
                        </div>
                        <button onClick={() => handleStageSubmit(1)} disabled={stage1Progress === 0 || !!nomineeData?.round2Unlocked} className="px-8 py-3 bg-blue-600 text-white font-bold rounded-2xl shadow-xl hover:shadow-blue-600/40 hover:-translate-y-1 transition-all disabled:opacity-30 flex items-center gap-2 text-xs uppercase tracking-widest truncate">Submit Stage 1</button>
                    </div>
                ...
                <div className={`rounded-3xl border transition-all duration-300 overflow-hidden ${nomineeData?.round3Unlocked ? 'bg-white border-gray-200 shadow-xl' : 'bg-gray-50 border-gray-100 opacity-60'}`}>
                    <button onClick={() => nomineeData?.round3Unlocked && setRound3Open(!round3Open)} disabled={!nomineeData?.round3Unlocked} className={`w-full p-8 flex items-center justify-between group transition-colors ${nomineeData?.round3Unlocked ? 'cursor-pointer hover:bg-gold-50/20' : 'cursor-not-allowed'}`}>
                        <div className="flex items-center space-x-6">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${nomineeData?.round3Unlocked ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-200 text-gray-400'}`}>{nomineeData?.round3Unlocked ? <Unlock size={24} /> : <Lock size={24} />}</div>
                            <div className="text-left">
                                <div className="flex items-center gap-3">
                        ...
                        {nomineeData?.round3Unlocked && (
                            <div className="flex items-center space-x-3 text-blue-600 bg-blue-50 px-5 py-2 rounded-2xl font-bold uppercase tracking-widest text-[10px] group-hover:bg-blue-600 group-hover:text-white transition-all"><span>{round3Open ? 'Hide' : 'Review'}</span>{round3Open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
                        )}
                    </button>
